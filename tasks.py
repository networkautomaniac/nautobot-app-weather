...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_WEATHER_xxx
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.weather.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.weather.compose_http_timeout,
        "NAUTOBOT_VER": context.weather.nautobot_ver,
        "PYTHON_VER": context.weather.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.weather.project_name}",
        f'--project-directory "{context.weather.compose_dir}"',
    ]

    for compose_file in context.weather.compose_files:
        compose_file_path = os.path.join(context.weather.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.weather.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.weather.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations weather"
...
@task(
    help={
        "version": "Version of Weather to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml weather'
    run_command(context, command)
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'weather/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.weather.local):
