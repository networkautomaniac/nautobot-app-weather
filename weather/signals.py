...
from weather.jobs import UpdateLocationWeather
...
def enable_job(apps, **kwargs):
    ...
    if config.weather__ENABLE_JOB:
        for job in Job.objects.filter(module_name__startswith="weather.jobs"):
            job.enabled = True
            job.validated_save()
